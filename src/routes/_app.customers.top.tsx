...
import { Trophy, Crown, Medal, Award, Download, ArrowRight, Search } from "lucide-react";
...
function TopCustomers() {
  ...
  const exportCsv = () => {
    const rows = ["الترتيب,الاسم,البريد,عدد الطلبات,إجمالي الإنفاق,متوسط الطلب,صافي الربح,آخر طلب"];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            رتّب عملاءك حسب الإنفاق أو عدد الطلبات لتحديد الأكثر ولاءً.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Kpi icon={Medal} label="متوسط الطلب" value={formatEGP(totals.totalOrders > 0 ? totals.totalSpend / totals.totalOrders : 0)} tone="info" hint="لكل طلب" />
      </div>
      ...
      <div className="surface-elevated overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">اضغط على اسم العميل لعرض ملفه الكامل</p>
          </div>
        </div>
        {filtered.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead className="border-t border-border bg-surface-sunken/60 text-xs text-muted-foreground">
                <tr>
                  ...
                  <th className="px-3 py-2.5 text-start font-medium">صافي الربح</th>
                  <th className="px-5 py-2.5 text-start font-medium">آخر طلب</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((c, i) => (
                  <tr key={c.id} className="border-t border-border/60 hover:bg-accent/30">
                    ...
                    <td className="px-3 py-3 num text-muted-foreground">{formatEGP(c.profit)}</td>
